...
function buildServer() {
  ...
  // --- NEW TOOL: reads Campaign Budget Scheduling ("high demand" period
  // increases) configured on a campaign. Meta exposes this via a dedicated
  // edge (/{campaign_id}/budget_schedules), not as a plain campaign field,
  // which is why meta_list_campaigns could never surface it. Read-only,
  // no ALLOW_MUTATIONS gate needed.
  server.tool(
    "meta_get_budget_schedule",
    "Read scheduled budget increases (Campaign Budget Scheduling / 'high demand' periods) configured on a campaign. Read-only.",
    {
      campaign_id: z.string().min(1),
      fields: z.string().default(
        "id,budget_value,budget_value_type,time_start,time_end,recurrence_type,status"
      )
    },
    async ({ campaign_id, fields }) =>
      result(await metaRequest("GET", `${campaign_id}/budget_schedules`, { fields }))
  );
  ...
  server.tool(
    ...
    async ({ campaign_id, name, daily_budget_cents, lifetime_budget_cents, billing_event, optimization_goal, targeting, promoted_object, attribution_spec, destination_type, start_time, end_time, confirmed }) => {
      const preview = {
        campaign_id, name, daily_budget_cents, lifetime_budget_cents,
        billing_event, optimization_goal, targeting, promoted_object, attribution_spec, destination_type, start_time, end_time,
  ...
  server.tool(
    ...
    "Update an existing ad set: name, budget, targeting, optimization goal, and/or status (ACTIVE/PAUSED). Only fields you pass are changed. Requires ALLOW_MUTATIONS=true and confirmed=true.",
    {
      ...
      destination_type: z.string().optional(),
      automatic_manual_state: z.enum(["AUTOMATIC", "MANUAL"]).optional(),
      status: z.enum(["ACTIVE", "PAUSED"]).optional(),
      confirmed: z.boolean().default(false)
    },
    async ({ adset_id, name, daily_budget_cents, lifetime_budget_cents, targeting, optimization_goal, destination_type, automatic_manual_state, status, confirmed }) => {
      const preview = { adset_id, name, daily_budget_cents, lifetime_budget_cents, targeting, optimization_goal, destination_type, automatic_manual_state, status };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      if (!confirmed) {
      ...
      if (destination_type) params.destination_type = destination_type;
      if (automatic_manual_state) params.automatic_manual_state = automatic_manual_state;
      if (status) params.status = status;
      if (Object.keys(params).length === 0) {
        return result({ executed: false, reason: "No fields provided to update." });
      }
  ...
  server.tool(
    "upload_ad_image",
    "Upload an image (base64-encoded JPEG/PNG bytes) to the ad account's image library. Returns an image_hash usable in create_adcreative's image_hash param, or as a video thumbnail. Requires ALLOW_MUTATIONS=true and confirmed=true.",
    {
      name: z.string().min(1),
      image_base64: z.string().min(1),
      confirmed: z.boolean().default(false)
    },
    async ({ name, image_base64, confirmed }) => {
      const preview = { name, bytes_length: image_base64.length };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      if (!confirmed) {
      ...
      const data = await metaRequest("POST", `${META_ACCOUNT}/adimages`, {
        bytes: image_base64
      });
      return result(data);
    }
  ...
  server.tool(
    "create_adcreative",
    "Create a new ad creative. Supports a single image/video link ad (image_hash or video_id), OR a carousel ad (pass carousel_cards: 2-10 cards, each with its own image_hash/link/name/description). Returns a creative id usable as creative_id in create_ad or update_ad. Requires ALLOW_MUTATIONS=true and confirmed=true.",
    {
      name: z.string().min(1),
      ...
      carousel_cards: z
        ...
        .max(10)
        .optional(),
      multi_share_end_card: z.boolean().default(true),
      confirmed: z.boolean().default(false)
    },
    async ({
      name,
      ...
      const preview = {
        name, page_id, message, link, link_title, call_to_action_type,
        image_hash, video_id, carousel_cards, multi_share_end_card
      };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      ...
      if (carousel_cards) {
        // Carousel format: object_story_spec.link_data.child_attachments.
        // Each card needs its own image_hash (upload each photo first via
        // upload_ad_image) and can carry its own link/name/description; a
        // card without its own link falls back to the top-level `link`.
        if (!link) {
          return result({ executed: false, reason: "A carousel requires a top-level `link` (used as the default destination for cards without their own link)." });
        }
        const child_attachments = carousel_cards.map(card => ({
          link: card.link || link,
      ...
      if (!image_hash && !video_id) {
        return result({ executed: false, reason: "Provide image_hash, video_id, or carousel_cards (2-10 cards) from existing, already-uploaded assets." });
      }
  ...
  server.tool(
    ...
    "Create a Custom Audience (e.g. website/pixel-event based) that can later be used as the source for a lookalike audience or for direct targeting. Pass a raw Meta 'rule' object for pixel/event-based audiences. Requires ALLOW_MUTATIONS=true and confirmed=true.",
    {
      name: z.string().min(1),
      subtype: z.string().default("WEBSITE"),
      description: z.string().optional(),
      rule: z.record(z.any()).optional(),
      retention_days: z.number().int().min(1).max(180).default(180),
      confirmed: z.boolean().default(false)
    },
    async ({ name, subtype, description, rule, retention_days, confirmed }) => {
      const preview = { name, subtype, description, rule, retention_days };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      if (!confirmed) {
      ...
      const params: Record<string, string> = { name, retention_days: String(retention_days) };
      if (description) params.description = description;
      if (rule) params.rule = JSON.stringify(rule);
      return result(await metaRequest("POST", `${META_ACCOUNT}/customaudiences`, params));
    }
  );

  server.tool(
    "create_lookalike_audience",
    "Create a Lookalike Audience from an existing custom audience (origin_audience_id). ratio is the target audience size, e.g. 0.03 = top 3% most similar in the chosen country. Requires ALLOW_MUTATIONS=true and confirmed=true.",
    {
      name: z.string().min(1),
      origin_audience_id: z.string().min(1),
      country: z.string().length(2).default("BR"),
      ratio: z.number().min(0.01).max(0.2).default(0.03),
      confirmed: z.boolean().default(false)
    },
    async ({ name, origin_audience_id, country, ratio, confirmed }) => {
      const preview = { name, origin_audience_id, country, ratio };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      if (!confirmed) {
