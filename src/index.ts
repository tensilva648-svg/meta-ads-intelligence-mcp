...
function buildServer() {
  ...
  server.tool(
    ...
    async ({ adset_id, name, daily_budget_cents, lifetime_budget_cents, targeting, optimization_goal, destination_type, status, confirmed }) => {
      const preview = { adset_id, name, daily_budget_cents, lifetime_budget_cents, targeting, optimization_goal, destination_type, status };
      if (!ALLOW_MUTATIONS) {
        return result({ executed: false, reason: "Mutations are disabled. Set ALLOW_MUTATIONS=true on the server after testing.", preview });
      }
      if (!confirmed) {
